package com.cabatuan.colorreduction;
...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener, Camera.PreviewCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                reductionBy = progress + 1;
                label.setText("   reductionBy = " + reductionBy);
    ...
    public native void process(Bitmap pTarget, byte[] pSource, int reductionBy);

    private Runnable DoImageProcessing = new Runnable() {
        public void run() {
            bProcessing = true;
            process(imageA, videoSource, reductionBy);
            imViewA.invalidate();
